...
def get_unit_uic_reason(file, fields):
    ...
    for line in file:
        ...
        if t:
            temp = file.readline()
            # print('temp: '+ temp)
            UIC_CD_loc = re.search(r" \w{6} ", temp)
            UIC_CD = UIC_CD_loc.group().replace(' ', '')
            # print('UIC_CD: ' + UIC_CD)
            EVAL_RPT_RSN_CD = temp[UIC_CD_loc.end():].replace('\n', '')
            # print('EVAL_RPT_RSN_CD ' + EVAL_RPT_RSN_CD)

            UNIT = temp[:UIC_CD_loc.start()]
